        ...
        while true
            ...
            for receiver in receivingUnits
                ...
                if (length(receiver.inputLocation.resources) + shipping.batchSize <= receiver.inputStorageSize)
                    ...
                    components_shipped = ceil(Int, shipping.batchSize * (1.0 - shipping.supplier.defectRate * rand()))
                    ...
                    # Only the working components are claimed while the faulty ones are removed using the
                ...
                if (success)
                    ...
                    model.data["total_shipping_time_with_delays"] += (shipping.receivers[receiver] + shippingTimeDelayed)
                    ...
                    faulty_components_quantity = shipping.batchSize - components_shipped
                    success_faulty_claim, faulty_claimed_tree = @claim(process, (shipping.supplier.outputLocation, 
                                                                SysModels.find(r -> typeof(r) == Component && r.name == shipping.componentShipped.name,
                                                                faulty_components_quantity)))

                    # Converting the tree generated by the @claim function to a list so that these defective
